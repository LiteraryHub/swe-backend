...
// Endpoint to fetch authors with first name equal to "نجيب"
...
// Endpoint for server-side search filtering and pagination for authors
app.get("/getauthorsdiscovers", async (req, res) => {
    const page = parseInt(req.query.page) || 1; // Current page number, default to 1 if not provided
    const limit = 5; // Maximum number of documents per page
    ...
    try {
        ...
        if (searchTerm !== '') {
            // If search term is provided, construct query to search by author name
            query = {
                $or: [
